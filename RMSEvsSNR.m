...
rRange = [10 100]; omegaRange = [0 2*pi]; phiRange = [0 pi/2];
...
results = struct( ...
    'config', config, 'truePolar', truePolar, ...
    'truePosition', truePosition, 'alpha', alpha, ...
    'apleInitialPosition', apInitial, 'estimate', estimate, ...
    'squaredError', squaredError, 'crbTrace', crbTrace, ...
    'crbFailure', {crbFailure}, ...
    'success', success, 'apleDiagnostics', {apDiagnostics}, ...
    'optimizationDiagnostics', {optimizationDiagnostics}, ...
    'rmse', rmse, 'status', 'complete');
